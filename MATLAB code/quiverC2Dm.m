...
if numel(u) > ncolors
    ...
    for k = 1:ncolors
        mask = (Ic == k);    
        quiver(ax, x, y, u, v, 0, ...
            'Color', 'b', 'LineWidth', lw, 'MaxHeadSize', hs);
    end
else
    ...
    % linear indexing!
    for k = 1:numel(u)
        quiver(ax, x(k), y(k), u(k), v(k), 0, 'Color', 'b', ...
...
if p.Results.Colorbar
    ...
    cb.Label.Position(1) = 4.2;
    ylabel(cb,'Normalized H Field (A/m)','FontSize',16,'Rotation',270) 
end
